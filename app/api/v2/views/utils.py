...
def product_valid(productname,description, category, quantity, price):
    if productname == '' or description =='' or category == '':
        return {"message":"productname,descriprion and category are required texts, please check your input"}, 400
    elif not isinstance(quantity, int) or isinstance(price, int) != True:
        return {"message":"kindly check your input, the price and quantity MUST be numbers"}
    elif re.findall(r'"(.*?)"', (productname,descriprion,category)):
        return {"message":"kindly look over your input"}
    elif isinstance(quantity, str) == True or  isinstance(price, str):
        try:
