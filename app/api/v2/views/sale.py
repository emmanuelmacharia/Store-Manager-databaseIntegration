...
class Sales(Resource):
    ...
    def post(self):
        ...
        record = cur.fetchone()
        if record == None:
            return {'message':'No product by that name in the inventory,check your input'}, 404
        elif quantity > int(record[4]):
            return{'message':'Request exeeds the quantity of the {} in our inventory'.format(productname)}, 400
        else:
            newsale = Sale.save(productname,price,quantity)
            return make_response(jsonify({'message':'sale made successfully', 'sale record':newsale}),201)
